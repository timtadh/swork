...
0.3). If you would like to install the master remove "@r0.4". Or simply update
aftwards with `swork update [-s] --release=master`.

    pip install psutil # this is not done automatically for some reason...
    pip install --src="$HOME/.src" -e git://github.com/timtadh/swork.git@r0.4#egg=swork
...
fact the command generated by `swork update`.
    ...
    pip install --src="$HOME/.src" --upgrade -e git://github.com/timtadh/swork.git@r0.4#egg=swork
